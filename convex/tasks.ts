...
export const getTasksByDate = query({
  ...
  handler: async (ctx, args) => {
    ...
    const tasks = await filter(
      ctx.db
        .query('tasks')
        .withIndex('userId', (q) => q.eq('userId', currentUser?.userId as Id<'users'>)),
      (task) => isSameDay(new Date(task.expireAt), new Date(args.day))
    ).collect();
...
export const updateTask = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    previousChildTasks.forEach(async (previousChild) => await ctx.db.delete(previousChild._id));
